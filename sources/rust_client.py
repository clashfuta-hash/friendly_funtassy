...
class RustClient:
    ...
    def seed_fixture(
        ...
        result = self._post("/games/seed", payload)  # NEW Rust endpoint
        if result is None:
            return False
    ...
    def get_fixtures_pending_resolution(self, source: str) -> List[Dict[str, Any]]:
        result = self._get("/games/pending-resolution", {"source": source})  # NEW
    ...
    def update_fixture_resolved(
        ...
        result = self._post(f"/games/{match_id}/resolve", payload)  # NEW
        return result is not None

    def abandon_fixture(self, match_id: str) -> bool:
        result = self._post(f"/games/{match_id}/abandon", {})  # NEW
        return result is not None
